...
Для основного сценария не нужны `sudo`, отдельный Unix user или Docker.

После запуска сервиса открой бота и отправь `/start`. Дальше ведут кнопки:
**Подключить Codex** открывает device login, если он нужен, **Проверить вход**
проверяет авторизацию, **Создать Groq key** опционально включает транскрипцию,
а **Начать первую задачу** завершает onboarding. После этого терминал для
обычной работы больше не нужен.
...
non-root container. Codex login всё равно проходит из бота; `./docker.sh login`
нужен только как аварийный fallback.
