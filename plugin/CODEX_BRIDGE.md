...
меняет код и runtime. Явная просьба внедрить обсуждённый вариант закрывает
карточный режим без специальной команды и продолжает тот же запрос с обычной
политикой проекта; отклонённые и невалидные карточки следующие сообщения не
перехватывают. Внутренние Git-ошибки остаются в durable operator state. Полный
