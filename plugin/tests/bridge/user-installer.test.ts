...
`)
    ...
    expect(stdout).toContain('Активация в Telegram')
    expect(stdout).toContain('привяжи себя владельцем')
    expect(stdout).toContain('Сервис установлен')
    expect(stdout).not.toContain('owner ID')
    expect(stdout).not.toContain('open the one-time link')
    expect(stdout).not.toContain(token)
    expect(stderr).not.toContain(token)
